...
void ShadowRenderer::update(irr::video::ITexture *outputTarget)
{
	...
	bool updatedrawlist = true;
	if (renderTargets.empty()) {
		if (!_enable_csm) {
			renderTargets.push_back(
						getSMTexture(std::string("shadow_0_") +
	...
	if (!ShadowNodeArray.empty() && !_light_list.empty()) {
		...
		for (DirectionalLight &light : _light_list) {
			...
			bool blah = false;
			for (int nSplit = 0; nSplit < light.getNumberSplits(); nSplit++) {
				_driver->setTransform(irr::video::ETS_VIEW,
						light.getViewMatrix(nSplit));
				_driver->setTransform(irr::video::ETS_PROJECTION,
						light.getProjectionMatrix(nSplit));
				_shadow_depth_cb->idx = nSplit;

				// right now we can only render in usual RTT, not Depth
				// texture is available in irrlicth
				// maybe we should put some gl* fn here
				_driver->setRenderTarget(renderTargets[nSplit], true,
						true,
				...
				for (const auto &shadow_node : ShadowNodeArray) {
					...
					if (std::string(shadow_node.node->getName()) ==
							"ClientMap") {

						ClientMap *map_node = static_cast<
								ClientMap *>(
								shadow_node.node);

						// Ok, we have to update the nodes we have
						// in the line of vision of our fake
						// light, aka shadow light to do that we
						// are going to use our custom updateDraw
						// fn.
						 //esto borra la lista de draw cada vez?
						/**/
						
							map_node->updateDrawListShadow(
									light.getPosition(
											nSplit),
									light.getDirection(),
									_shadow_map_max_distance*BS);
						...
						if (map_node->getMaterialCount() > 0) {
							...
							// material, which is the one with
							// the albedo info ;)
							material = map_node->getMaterial(
									0);
						 }
						...
						// IDK if we need the back face culling...
						...
						_driver->setTransform(
								irr::video::ETS_WORLD,
								map_node->getAbsoluteTransformation());

						map_node->renderMapShadows(_driver,
								material,
				...
				for (u32 m = 0; m < n_node_materials; m++) {
					...
					current_mat.MaterialType =
							(irr::video::E_MATERIAL_TYPE)
									depth_shader;
					BufferMaterialCullingList.push_back(std::make_pair<
							bool, bool>(
				...
				for (u32 m = 0; m < n_node_materials; m++) {
					...
					current_mat.BackfaceCulling =
							BufferMaterialCullingList[m].first;
					current_mat.FrontfaceCulling =
							BufferMaterialCullingList[m]
									.second;
			...
			// If CMS we should make a second pass :(
			if (_enable_csm) {
				...
				_driver->setRenderTarget(renderTargets[3], true, true);
...
if(_enable_csm) {
		...
		_driver->draw2DImage(_enable_csm ? renderTargets[3] : renderTargets[0],
		...
		_driver->draw2DImage(renderTargets[1],
				irr::core::rect<s32>(0, 50 + 256, 128, 128 + 50 + 256),
				irr::core::rect<s32>(0, 0,
						renderTargets[0]->getSize().Width,
						renderTargets[0]->getSize().Height));
...
{
	return _enable_csm ? renderTargets[3]:renderTargets[0];
}
 ...
 * I used a custom loader because I couldn't figure out how to use the base Shaders system
 * with custom IShaderConstantSetCallBack without messing up the code too much.
 * If anyone knows how to integrate this with the standard MT shaders, please feel free to
 * change it.
 */
...
void ShadowRenderer::createShaders()
{
	...
	if (depth_shader == -1) {
		...
		if (depth_shader_vs.empty()) {
			_shadows_enabled = false;
			_device->getLogger()->log(
					"Error shadow mapping vs shader not found.",
					ELL_WARNING);
			return;
		}
		...
		if (depth_shader_fs.empty()) {
			_shadows_enabled = false;
			_device->getLogger()->log(
					"Error shadow mapping fs shader not found.",
					ELL_WARNING);
			return;
		}
	...
	if (_enable_csm && mixcsm_shader == -1) {
		...
		if (mixcsm_shader == -1) {
			...
			_shadows_enabled = false;
			_device->getLogger()->log(
					"Error compiling cascade shadow mapping shader.",
					ELL_WARNING);
			return;
		}
